	...
	useEffect(() => {
		...
		canvasRef.current?.loadFromJSON(json, async () => {
			...
			const fontPromises = artboardTexts?.map((item: any) => {
				...
				if (
					textItem.data &&
					typeof textItem.data.weightGoogleFont === 'string' &&
					typeof textItem.fontFamily === 'string'
				) {
					const weightGoogleFont = textItem.data.weightGoogleFont;
					console.log('weightGoogleFont', weightGoogleFont, textItem.fontFamily);
					const style = document.createElement('style');

					style.appendChild(document.createTextNode(weightGoogleFont));
					document.head.appendChild(style);
				}
				if (
					textItem.data &&
					typeof textItem.data.googleFont === 'string' &&
					typeof textItem.fontFamily === 'string'
				) {
					const googleFont = textItem.data.weightGoogleFont;
					console.log('googleFont', googleFont, textItem.fontFamily);
					const style = document.createElement('style');

					style.appendChild(document.createTextNode(googleFont));
					document.head.appendChild(style);
				}
