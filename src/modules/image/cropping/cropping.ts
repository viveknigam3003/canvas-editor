...
// import { fabric.Canvas, fabric.Path, fabric.Point, fabric.util } from 'fabric/fabric-impl';
import { fabric } from 'fabric';
import { TPointerEvent, TPointerEventInfo } from './types';
import { CLIPPATHS, ClipPathType } from './clipPath';
import { containsPoint } from './utils';
import { fireCropImageEvent } from './handlers';
...
function canvasMouseDown(e: TPointerEventInfo<TPointerEvent>) {
	...
	if ((!target || activeObject.data.id !== target.data.id) && activeObject.isCropping) {
		...
		if (activeObject.cropPath) {
			const clipPath = new fabric.Path(activeObject.cropPath);
